...
{ 
	...
	static int length=5;
	static double d,dist,snake_taste=0;
	Thread t;
	...
	static volatile double turn=0;
	static volatile double flag[];
   ...
   !!*****!! 
	...
	void paint_applet(){
		...
		else 
		{
			...
			if(key1!=null)removeKeyListener(key1);
			if(key2!=null)removeKeyListener(key2);
			if(GUI.Music.getState())try{Vibrations.stopMusic();}catch(Exception eee){}
			if(GUI.Sounds.getState())if(Vibrations.p1!=null)try{Vibrations.p1.stop();}catch(Exception eee){}
	...
	void start_game_basic(){
		...
		flag=new double[2];flag[0]=flag[1]=0;
		//  setSize(400,400);
		if((GUI.Music.getState())){MainFrame.vb.playMusic();}
	...
	void processKeyMoveSimple(){
		...
		if((count==1)&&(GUI.Advance.isSelected()==false))
			key2=new innerKeyProcess();
		addKeyListener(key2);
	}


	class innerKeyProcess extends KeyAdapter{
		...
		public void keyPressed(KeyEvent ke){
			...
			if(GUI.Classic.isSelected()){
				...
				//check to stop next keySignal before this method is executed completely;
				if(key2!=null) {removeKeyListener(key2);key2=null;}
				...
				flag[1]=0;
				//Again, if game does not end, reattach key listener
				if(count==1) {
					processKeyMoveSimple();
